...
#include <XorShift.h>
void FourthMap::Initialize(DirectXCommon* dxCommon) {
	...
	//ステージマップ
	modelFourthMap = Model::CreateFromOBJ("StartMap");
	objFourthMap = TouchableObject::Create(modelFourthMap);
	objFourthMap->SetPosition({ 0,-1,2 });
	objFourthMap->SetRotation({ 0, 90, 0 });
	objFourthMap->SetScale({ 1.4f,1.5f,1.6f });
	//バリケード
...
void FourthMap::Update(DirectXCommon* dxCommon) {
	if (pause) {
		particleMan->Update();
		for (std::size_t i = 0; i < spawing.size(); i++) {
			spawing[i]->Update();
...
void FourthMap::Draw(DirectXCommon* dxCommon) {
	ImGui::Begin("test");
...
void FourthMap::Pause(const int& Timer) {
	wait++;
	if (wait >= Timer) {
		pause = false;
		wait = 0;
